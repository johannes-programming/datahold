...
test: beautiful dist
	...
	conda run -n test_datahold python run_tests.py;
	conda run -n test_datahold pip install mypy >/dev/null;
	conda run -n test_datahold python -m mypy --exclude build --exclude dist --strict .;
	conda run -n test_datahold python -m mypy --strict -p datahold;
