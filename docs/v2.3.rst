...
.. container:: warning
    ...
    **Warning:** The major version ``2`` of ``datahold`` has reached end-of-life.
...
that cannot change the underlying instance
...
.. code-block:: python
    ...
    def __format__(self: Self, format_spec: Any, /) -> Any:
        ...
        return dict[Key, Value](self.data).__format__(format_spec)
...
It implements all methods and classmethods of dict that can change the underlying instance (and that are not already present in ``object``).
